                                        # SQL Daily Progress

## Day 1 (18-07-2026)
    - ✅ LeetCode 175 - Combine Two Tables
    -     Topics Used: LEFT JOIN.

## Day 2 (19-07-2026)
    - ✅ LeetCode 595 - Big Countries
    -     Topics Used: WHERE and OR.

## Day 3 (20-07-2026)
    - ✅ LeetCode 1683 - Invalid Tweets
    -     Topics Used: WHERE and LENGTH() function.

## Day 4 (21-07-2026)
    - ✅ LeetCode 584 - Find Customer Referee
    -     Topics Used: WHERE, OR and IS NULL.

## Day 5 (22-07-2026)
    - ✅ LeetCode 1148 - Article Views I
    -     Topics Used: WHERE, DISTINCT and ORDER BY.

## Day 6 (23-07-2026)
    - ✅ LeetCode 1873 - Calculate Special Bonus
    -     Topics Used: Conditional Logic (CASE WHEN/IF).

## Day 7 (24-07-2026)
    - ✅ LeetCode 1068 - Product Sales Analysis I
    -     Topics Used: INNER JOIN.

## Day 8 (25-07-2026)
    - ✅ LeetCode 1378 - Replace Employee ID With The Unique Identifier
    -     Topics Used: LEFT JOIN.

## Day 9 (26-07-2026)
    - ✅ LeetCode 577 - Employee Bonus
    -     Topics Used: LEFT JOIN, WHERE and NULL Handling.

## Day 10 (27-07-2026)
    - ✅ LeetCode 1581 - Customer Who Visited but Did Not Make Any Transactions
    -     Topics Used: LEFT JOIN, WHERE, GROUP BY, COUNT() and NULL Handling.

## Day 11 (28-07-2026)
    - ✅ LeetCode 596 - Classes With at Least 5 Students
    -     Topics Used: GROUP BY, HAVING and COUNT().

## Day 12 (29-07-2026)
    - ✅ LeetCode 586 - Customer Placing the Largest Number of Orders
    -     Topics Used: GROUP BY, ORDER BY, COUNT() and LIMIT.

## Day 13 (30-07-2026)
    - ✅ LeetCode 1729 - Find Followers Count
    -     Topics Used: COUNT(), GROUP BY and ORDER BY.

    - ✅ HackerRank 01 - Revising the Select Query I
    -     Topics Used: SELECT, WHERE and AND.

## Day 14 (31-07-2026)
    - ✅ LeetCode 511 - Game Play Analysis I 
    -     Topics Used: MIN() and GROUP BY.

    - ✅ HackerRank 02 - Revising the Select Query II
    -     Topics Used: SELECT, WHERE and AND.

## Day 15 (01-08-2026)
    - ✅ LeetCode 1484 - Group Sold Products By The Date 
    -     Topics Used: COUNT(), GROUP_CONCAT(), DISTINCT, SEPARATOR, GROUP BY and ORDER BY.

    - ✅ HackerRank 03 - Select All
    -     Topics Used: SELECT.

## Day 16 (02-08-2026)
    - ✅ LeetCode 620 - Not Boring Movies
    -     Topics Used: WHERE, AND and ORDER BY.

    - ✅ HackerRank 04 - Select By ID
    -     Topics Used: SELECT and WHERE.

## Day 17 (03-08-2026)
    - ✅ LeetCode 607 - Sales Person
    -     Topics Used: LEFT JOIN, GROUP BY, HAVING, COALESCE() and SUM().

    - ✅ HackerRank 05 - Japanese Cities' Attributes
    -     Topics Used: SELECT and WHERE.

## Day 18 (04-08-2026)
    - ✅ LeetCode 1280 - Students and Examinations
    -     Topics Used: CROSS JOIN, LEFT JOIN, GROUP BY, ORDER BY and COUNT().

    - ✅ HackerRank 06 - Japanese Cities' Names
    -     Topics Used: SELECT and WHERE.

## Day 19 (05-08-2026)
    - ✅ LeetCode 619 - Biggest Single Number
    -     Topics Used: SubQuery( Derived Table), GROUP BY, HAVING, COUNT() and MAX().

    - ✅ HackerRank 07 - Weather Observation Station 1
    -     Topics Used: SELECT.

## Day 20 (06-08-2026)
    - ✅ LeetCode 1693 - Daily Leads and Partners
    -     Topics Used: COUNT(), DISTINCT and GROUP BY

    - ✅ HackerRank 08 - Weather Observation Station 3
    -     Topics Used: SELECT, DISTINCT, WHERE, MOD().

## Day 21 (07-08-2026)
    - ✅ LeetCode 1251 - Average Selling Price
    -     Topics Used: COALESCE(), ROUND(), SUM(), LEFT JOIN, BETWEEN and GROUP BY

    - ✅ HackerRank 09 - Weather Observation Station 4
    -     Topics Used: SELECT, COUNT() and DISTINCT.

## Day 22 (08-08-2026)
    - ✅ LeetCode 1757 - Recyclable and Low Fat Products
    -     Topics Used: SELECT, WHERE and AND

    - ✅ HackerRank 10 - Weather Observation Station 5
    -     Topics Used: SELECT, LENGTH(), ORDER BY and LIMIT.

